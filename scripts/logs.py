...
def extract_from_log(logfile, solver_name):
    ...
    if solver_name == "gurobi":
        ...
        raise NotImplementedError(
            "Log extraction not defined for solver {}".format(solver_name)
        )
...
def extract_from_log_gurobi(logfile):
    ...
    m = re.search(
        r"Optimize a model with (\d+) rows, (\d+) columns and (\d+) nonzeros", logs
    )
    if m is not None:
    ...
    ms = re.findall(
        r"Variable types: (\d+) continuous, (\d+) integer \((\d+) binary\)", logs
    )
    if ms is not None:
        pre = ""
        for m in ms:
    ...
    m = re.search(
        r"Best objective {r}, best bound {r}, gap {r}%".format(r=rfloate), logs
    )
    if m is not None:
...
if __name__ == "__main__":
    ...
    peak_mem = (
        pd.read_csv(
            snakemake.input.memory, sep=" ", index_col=1, usecols=[1, 2], header=None
        )
        .max()
        .values[0]
    )
